...
Below we analyze the frequency table of that groups in the solved-cases.csv dataset and estimate it with 
```{r relative freqs, echo=TRUE}
?boot
dif <- Solved$Group

sample_freqs = function(Solved, indices){
 sample = Solved[indices, ]
 bar = count(sample)
 return(bar)
}

boot(dif, statistic = sample_freqs, R = 5000)

```

Before running the model, we evaluate the uncertainity about the data using non-parametric bootstrapping.
```{r relative freqs, echo=TRUE}
